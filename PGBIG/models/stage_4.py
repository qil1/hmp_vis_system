...
import math
from PGBIG.models import BaseModel as BaseBlock
import PGBIG.utils.util as util
import numpy as np
import torch.nn.functional as F
from torch.nn.parameter import Parameter
from PGBIG.utils.opt import Options
...
class MultiStageModel(Module):
    def __init__(self, opt):
        ...
        # print("self.dct_n:", self.dct_n)  # 35
        # print(vars(opt))
        self.gcn_encoder1 = BaseBlock.GCN_encoder(in_channal=3, out_channal=self.d_model,  # d_model = 16
                                                  node_n=self.node_n,  # 22
                                                  seq_len=self.dct_n,  # 35 = 10 + 25
                                                  p_dropout=opt.drop_out,
                                                  num_stage=self.encoder_layer_num)  # 1
    ...
    def forward(self, src, input_n=10, output_n=10, itera=1):
        ...
        bs = src.shape[0]
        # print("src.shape =",src.shape)   [b, 35, 66]
        dct_n = self.dct_n
        idx = list(range(self.kernel_size)) + [self.kernel_size - 1] * output_n
        # print("idx =", idx)  # [0, 1, 2, ..., 9| 9, 9, ..., 9]
        input_gcn = src[:, idx].clone()  # 前10帧为gt，后25帧为第10帧重复25次

        dct_m, idct_m = util.get_dct_matrix(input_n + output_n)
        dct_m = torch.from_numpy(dct_m).float().to(self.opt.cuda_idx)  # [35, 35]
        idct_m = torch.from_numpy(idct_m).float().to(self.opt.cuda_idx)  # [35, 35]
        ...
        # stage1
        latent_gcn_dct = self.gcn_encoder1(input_gcn_dct)  # [b, 3, 22, 35] -> [b, 16, 22, 35]
        latent_gcn_dct = torch.cat((latent_gcn_dct, latent_gcn_dct), dim=3)  # [b, 16, 22, 35] -> [b, 16, 22, 70]
        output_dct_1 = self.gcn_decoder1(latent_gcn_dct)[:, :, :, :dct_n]  # [b, 16, 22, 70] -> [b, 3, 22, 35]
        ...
        output_dct_1 = output_dct_1.permute(0, 2, 1, 3).reshape(bs, -1, dct_n)  # [b, 66, 35]
        output_dct_2 = output_dct_2.permute(0, 2, 1, 3).reshape(bs, -1, dct_n)  # [b, 66, 35]
        output_dct_3 = output_dct_3.permute(0, 2, 1, 3).reshape(bs, -1, dct_n)  # [b, 66, 35]
        output_dct_4 = output_dct_4.permute(0, 2, 1, 3).reshape(bs, -1, dct_n)  # [b, 66, 35]

        output_1 = torch.matmul(idct_m[:, :dct_n], output_dct_1.permute(0, 2, 1))  # [b, 66, 35] -> [b, 35, 66]
